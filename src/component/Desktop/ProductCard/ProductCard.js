...
class ProductCard extends PureComponent {
    ...
    render() {
            ...
            return <Card className="product-card" key={i}>
                <div onClick={() => this.onGetProduct(pd.slug)}>
                    ...
                    <Card.Body className="product-info-div">
                        <Card.Title><p
                            className="productTitle">{pd.product_name.length > 30 ? pd.product_name.substring(0, 30) + "..." : pd.product_name}</p>
                        </Card.Title>
                        <Card.Text>
                            <div className="priceDiv">
                                {symbolFormat === 1 ?
                                    ...
                                    <Fragment>
                                        {pd.discount_price !== undefined ?
                                            <Fragment>
                                                <span
                                                    className="discountPrice">{defaultCurrency}{onCurrencyFormat(pd.discount_price)}</span>
                                                <span className="regularPrice ml-3 mr-3"><del>{defaultCurrency}{onCurrencyFormat(pd.product_price)}</del></span>
                                            </Fragment> :
                                            <Fragment>
                                                <span
                                                    className="regularPrice singlePrice ml-3 mr-3">{defaultCurrency}{onCurrencyFormat(pd.product_price)}</span>
                                            </Fragment>
                                        }
                                    </Fragment>
                                }
                ...
                <Card.Footer>
                    <p onClick={() => this.onWishList(pd.index)} id={`wishlist-${pd.index}`}
                       className="card-icons  pci1"><i className="far fa-heart"/></p>
                    <p onClick={() => this.onCompare(pd.index)} className="card-icons pci2"><i
                        className="far fa-sync-alt"/></p>
                    <p onClick={() => this.onCart(pd.index)} className="card-icons pci3"><i
                        className="far fa-shopping-cart "/></p>
                </Card.Footer>
            </Card>
        })
        ...
        let productForSlider = data.map((pd, i) => {
            return <Col key={i} xl={2} md={3} sm={4} xs={6} className="column-lg-5">
        ...
        let searchProduct = data.map((pd, i) => {
            return <Col key={i} xl={3} lg={3} md={4} sm={4} xs={4}>
                <Card className="product-card" key={i}>
                    <div onClick={() => this.onGetProduct(pd.slug)}>
                        <div className="cardImgDiv">
                            <div className="next-image-div cardImg">
        ...
        let product = data.map((pd, i) => {
            return <Col key={i} xl={2} lg={2} md={3} sm={4} xs={6}>
                <Card className="product-card" key={i}>
                    <div onClick={() => this.onGetProduct(pd.slug)}>
                        <div className="cardImgDiv  ">
                            <div className="next-image-div cardImg">
                        ...
                        <Card.Body className="product-info-div">
                            <Card.Title><p
                                className="productTitle">{pd.product_name.length > 20 ? pd.product_name.substring(0, 20) + "..." : pd.product_name}</p>
                            </Card.Title>
                            <Card.Text>
                                <div className="priceDiv">
                                    {symbolFormat === 1 ?
                                ...
                                <Row>
                                    <Col className="d-flex justify-content-between">
                                        <div className="TitleDiv">
                                            <h1 className="productCategory title">{this.props.title}</h1>
                                        </div>
                                        {this.props.loading !== true &&
                                            <Fragment>
                                              ...
                                              <span id="clockDiv" className="megaClock">
                                                <Countdown onComplete={this.onCompleteTimer}
                                        ...
                                        <div>
                                            {this.props.link != undefined &&
                                                <Link href={{pathname: `${this.props.link}`,}} className="btn">Show
                                                    All</Link>}
                                        </div>
                                    </Col>
                                </Row>
                                        ...
                                        {data.length <= 6 ?
                                            ...
                                            <Fragment>
                                                <div className="Slider">
                                                    <Button className="sliderLeftArrow sliderArrow"
                                                            onClick={this.previous}><i
                                                        className="far fa-chevron-left"/>
                                                    </Button>
                                                    <Button className="sliderRightArrow sliderArrow"
                                                            onClick={this.next}><i
